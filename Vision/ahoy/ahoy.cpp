...
int main()
{
  ...
  cv::Mat imgResize; //image resized to 320x240
	cv::Mat rightImgResize; //image resized to 320x240
  	...
  	for (;;)
		{
			...
			cv::gpu::resize(src, resize, cv::Size(leftCamera.get(CV_CAP_PROP_FRAME_WIDTH) / img_scale_factor, leftCamera.get(CV_CAP_PROP_FRAME_HEIGHT) / img_scale_factor), CV_INTER_CUBIC);
